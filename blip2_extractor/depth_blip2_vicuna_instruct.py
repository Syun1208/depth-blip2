...
from blip2_extractor.modeling_llama import LlamaForCausalLM
from blip2_extractor.blip2 import Blip2Base, disabled_train
from common.registry import registry

@registry.register_model("depth_blip2_vicuna_instruct")
class Blip2VicunaInstruct(Blip2Base):
    ...
    def __init__(
        ...
        llm_model = "./config/blip2-vicuna-instruct-7b/vicuna-7b"
    ...
    def extract_features(
        ...
        Returns:
            features (vector): A features of BLIP-2.
        """
        image = samples["image"]
        caption = samples.get("text_input")
        prompt = samples.get("prefix_prompt", None)
        # assert mode is one of "image", "text", "multimodal"

        assert (
                image is not None and caption is not None
        ), "Image / Capation is not provided for mode 'image' or 'multimodal'"
        ...
        with self.maybe_autocast():
            ...
            if prompt:
                text = prompt + self.positional_embedding.type(self.dtype)  # (n_cls, 1+n_ctx+*, ctx_dim(512))
                text = text + caption
            else:
            ...
            outputs_opt = query_output.last_hidden_state[:, : query_tokens.size(1), :]
            return outputs_opt
